...
def fertilizer_requirement(crop, area, nitrogen, phosphorus, potassium):
    fertilizer = []
    if crop == 'Soybean' :
        if phosphorus < 60:
        ...
        temp = round(200*area,2)
        st.write(f"1. foiler spray of 2% concentration Potassium Nitrate: {temp} litres")
    elif crop == 'Tomato' or crop == 'Chilli' :
...
def advisoryhome() :
    ...
    if selected == 'Fertilizer Advisory' :
        ...
        if check :
            st.success("Submitted")
            fertilizer_requirement(crop, area, nitrogen, phosphorus, potassium)
...
def yieldprediction() : 
    ...
    if crop == 'Soybean':
        ...
        if check : 
            ...
            x = prediction[0][0] if prediction[0][0] > 0 else 0
